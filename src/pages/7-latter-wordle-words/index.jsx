...
import { SevenDictionary } from '../../data/SevenDictionary';
import { isValidInput } from '../../function/isValidInput';

export const metadata = {
  title: 'Wordle/7-latter game',
...
export default function Page() {
 ...
 const fixedClassD =
   'flex select-none items-center justify-center border-2 border-gray-30 text-3xl font-bold uppercase text-black dark:border-black-10 dark:text-white';
    ...
    //get random index
  const index = getRndInteger(0, SevenDictionary.length);

  // set 7 word random value in the local storage
  useEffect(() => {
    localStorage.setItem('word', SevenDictionary[index].toUpperCase());
  }, []);
...
toast.success(`Your guess is right! The word is ${getWordFromLocalStorage}`);
          ...
          if(SevenDictionary.indexOf(row1String) === -1){
      ...
      } else if (row2.length == 7 && enter1 && row3.length != 7) {
        setEnter2(true);
...
toast.success(`Your guess is right! The word is ${getWordFromLocalStorage}`);
          ...
          if(SevenDictionary.indexOf(row2String) === -1){
            toast.error(`Your guess is not in the dictionary`);
            setEnter2(false);
          }
          else{ 
      ...
      } else if (row3.length == 7 && enter1 && enter2 && row4.length != 7) {
        setEnter3(true);
...
toast.success(`Your guess is right! The word is ${getWordFromLocalStorage}`);
          ...
          if(SevenDictionary.indexOf(row3String) === -1){
            toast.error(`Your guess is not in the dictionary`);
            setEnter3(false);
          }
          else{ 
...
toast.success(`Your guess is right! The word is ${getWordFromLocalStorage}`);
          ...
          if(SevenDictionary.indexOf(row4String) === -1){
            toast.error(`Your guess is not in the dictionary`);
            setEnter4(false);
          }
          else{ 
...
toast.success(`Your guess is right! The word is ${getWordFromLocalStorage}`);
          ...
          if(SevenDictionary.indexOf(row5String) === -1){
            toast.error(`Your guess is not in the dictionary`);
            setEnter5(false);
          }
          else{ 
...
toast.success(`Your guess is right! The word is ${getWordFromLocalStorage}`);
      ...
      if (row2.length <= 7 && row1.length == 7 && enter1) {
       ...
       <div className="mx-auto flex h-app-content w-full max-w-lg flex-col items-center">
        <div className="display">
          <div className="flex w-full flex-grow items-center justify-center">
            <div
              className={`grid h-[372px] w-80 max-w-xs grid-cols-7 grid-rows-6 gap-[5px] p-2`}
            >
              {/* row1 */}
              <div
